    ...
    <a-row :gutter="18">
      <a-col :span="16">
        ...
        <a-form layout="inline" class="j-inline-form">
          ...
          <!-- 动态生成的查询条件 -->
          <f-select-dev-query-item v-if="queryConfig.length>0" v-show="showMoreQueryItems" :queryParam="queryParam" :queryConfig="queryConfig" @pressEnter="searchQuery"/>
          <!-- 按钮 -->
...
<script>
  ...
  import FSelectDevQueryItem from './FSelectDevQueryItem'

  export default {
    name: 'FSelectDevComponentModal',
    mixins: [JeecgListMixin],
    components: {Ellipsis, FSelectDevQueryItem},
    props: {
      value: {
        type: Array,
        default: () => []
