...
import org.fleet.modules.demo.test.mapper.FleetDemoMapper;
 ...
 */
@Service
public class FleetDemoServiceImpl extends ServiceImpl<FleetDemoMapper, FleetDemo> implements IFleetDemoService {
	@Autowired
	FleetDemoMapper fleetDemoMapper;
	...
	public void testTran() {
		FleetDemo pp = new FleetDemo();
		pp.setAge(1111);
		pp.setName("测试事务  小白兔 1");
		fleetDemoMapper.insert(pp);
		
		FleetDemo pp2 = new FleetDemo();
		pp2.setAge(2222);
		pp2.setName("测试事务  小白兔 2");
		fleetDemoMapper.insert(pp2);
		
		Integer.parseInt("hello");//自定义异常
		
		FleetDemo pp3 = new FleetDemo();
		pp3.setAge(3333);
		pp3.setName("测试事务  小白兔 3");
		fleetDemoMapper.insert(pp3);
		return ;
	}
	...
	public FleetDemo getByIdCacheable(String id) {
		FleetDemo t = fleetDemoMapper.selectById(id);
	...
	@Override
	public IPage<FleetDemo> queryListWithPermission(int pageSize,int pageNo) {
		Page<FleetDemo> page = new Page<>(pageNo, pageSize);
		//编程方式，获取当前请求的数据权限规则SQL片段
		String sql = QueryGenerator.installAuthJdbc(FleetDemo.class);
	...
	public String getExportFields() {
		...
		//查询配置菜单有效字段
		List<String> allAuth = this.fleetDemoMapper.queryAllAuth(permsPrefix);
		//查询已授权字段
		List<String> userAuth = this.fleetDemoMapper.queryUserAuth(sysUser.getId(),permsPrefix);
		//列出未授权字段
		...
		Field[] fileds = FleetDemo.class.getDeclaredFields();
