...
import org.fleet.common.aspect.annotation.PermissionData;
...
import org.fleet.common.util.RedisUtil;
...
public class FleetDemoController extends FleetController<FleetDemo, IFleetDemoService> {
    @Autowired
    private IFleetDemoService fleetDemoService;
    ...
    public Result<?> list(FleetDemo fleetDemo, @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo, @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize,
                          HttpServletRequest req) {
        QueryWrapper<FleetDemo> queryWrapper = QueryGenerator.initQueryWrapper(fleetDemo, req.getParameterMap());
    ...
    public Result<?> add(@RequestBody FleetDemo fleetDemo) {
        fleetDemoService.save(fleetDemo);
    ...
    public Result<?> edit(@RequestBody FleetDemo fleetDemo) {
        fleetDemoService.updateById(fleetDemo);
    ...
    public Result<?> delete(@RequestParam(name = "id", required = true) String id) {
        fleetDemoService.removeById(id);
    ...
    public Result<?> deleteBatch(@RequestParam(name = "ids", required = true) String ids) {
        this.fleetDemoService.removeByIds(Arrays.asList(ids.split(",")));
    ...
    public Result<?> queryById(@ApiParam(name = "id", value = "示例id", required = true) @RequestParam(name = "id", required = true) String id) {
        FleetDemo fleetDemo = fleetDemoService.getById(id);
        return Result.OK(fleetDemo);
    }
    ...
    public Result<?> importExcel(HttpServletRequest request, HttpServletResponse response) {
        return super.importExcel(request, response, FleetDemo.class);
    }
    ...
    public FleetDemo redisGetFleetDemo(@PathVariable("id") String id) {
        FleetDemo t = fleetDemoService.getByIdCacheable(id);
        log.info(t.toString());
        return t;
    }
    ...
    @PermissionData(pageComponent = "jeecg/JeecgDemoList")
    public Result<?> loadSqlPermissonList(FleetDemo fleetDemo, @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo, @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize,
                                          HttpServletRequest req) {
        IPage<FleetDemo> pageList = fleetDemoService.queryListWithPermission(pageSize, pageNo);
        return Result.OK(pageList);
    }
    /*----------------------------------------外部获取权限示例------------------------------------*/
}
