...
import org.fleet.modules.message.websocket.WebSocket;
import org.fleet.modules.system.entity.*;
import org.fleet.modules.system.mapper.*;
import org.fleet.modules.system.service.*;
...
public class SysBaseApiImpl implements ISysBaseAPI {
	...
	public void sendTemplateAnnouncement(TemplateMessageDTO message) {
		...
		if(sysSmsTemplates==null||sysSmsTemplates.size()==0){
			throw new FleetBootException("消息模板不存在，模板编码："+templateCode);
		}
		...
		if("".equals(DB_TYPE)) {
			...
			try {
				...
				if(dbType.indexOf("mysql")>=0) {
					...
					throw new FleetBootException("数据库类型:["+dbType+"]不识别!");
				}
			} catch (Exception e) {
